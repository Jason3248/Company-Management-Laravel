    ...
    <div class="max-w-xl mx-auto bg-white p-6 rounded shadow">
        ...
        <form action="{{ route('companies.update', $company->id) }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $company->name) }}" class="w-full mt-1 p-2 border border-gray-300 rounded" required>
            </div>

            <div>
                <label class="block text-sm font-medium">Location</label>
                <input type="text" name="location" value="{{ old('location', $company->location) }}" class="w-full mt-1 p-2 border border-gray-300 rounded" required>
            </div>
